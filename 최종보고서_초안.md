...
![JOIN 결과 - 보유 학생 현황](screenshots/10_cultivation_cost.png)
...
![Flet GUI - 학생 도감 탭](screenshots/01_student_list.png)
...
![이미지 출력 - 학생 초상화](screenshots/04_student_detail.png)
...
![GitHub Public Repository](screenshots/11_github_repo.png)
...
> 설계서 작성 시 직접 그린 화면 구성도 (구현 전 설계 단계 그림, 실제 구현 화면은 4장 참고)

![전체 화면 구조](<4.1 전체 화면 구조.png>)
![학생 도감 화면 설계](<4.2 학생 도감 화면.png>)
![가챠 시뮬레이터 화면 설계](<4.3 가챠 시뮬레이터.png>)
![육성 시뮬레이터 화면 설계](<4.4 육성.png>)
...
![학생 도감 탭 실행 화면](screenshots/01_student_list.png)
![이름 검색(필터링) 적용 화면](screenshots/04_student_detail.png)
...
![학생 상세 정보 + 초상화 이미지](screenshots/04_student_detail.png)
![스킬 버튼 클릭 후 AlertDialog 팝업 (아이콘+한글 번역)](screenshots/05_skill_dialog.png)
...
def make_skill_handler(label, name, d1, dm, icon_url):
    ...
    def handler(e):
        ...
        # 다이얼로그 제목에도 스킬 아이콘 이미지를 텍스트와 함께 표시
        title_items = []
        if icon_url:
            title_items.append(ft.Image(src=icon_url, width=24, height=24,
                                         error_content=ft.Container(width=24, height=24)))
        title_items.append(ft.Text(f"[{label}] {name}", weight=ft.FontWeight.BOLD))
...
![가챠 탭 초기 화면](screenshots/06_gacha_initial.png)
![10회 뽑기 후 결과 화면](screenshots/07_gacha_pulled.png)
...
![육성 시뮬레이터 - 보유 학생 목록](screenshots/08_cultivation_owned.png)
...
![보유 학생 선택 후 현재/목표 수치 폼](screenshots/09_cultivation_form.png)
![비용 계산 결과 화면](screenshots/10_cultivation_cost.png)
...
**전체 소스 코드(직접 작성한 .py 파일 27개)는 부록 A에 텍스트로 첨부 (Ctrl+F 검색 가능)**
...
### A.4 repository/duckdb/__init__.py

```python
# repository/duckdb 패키지 초기화
```

### A.5 repository/duckdb/connection.py
...
### A.6 repository/duckdb/student.py
...
### A.7 repository/duckdb/student_image.py
...
### A.8 repository/duckdb/student_stat.py
...
### A.9 repository/duckdb/skill.py
...
### A.10 repository/duckdb/banner.py
...
### A.11 repository/duckdb/gacha_pull.py
...
### A.12 repository/duckdb/cultivation.py
...
### A.13 repository/duckdb/cultivation_goal.py
...
### A.14 repository/duckdb/join.py
...
### A.16 service/student_service.py
...
### A.18 service/cultivation_service.py
...
### A.24 views/cultivation_view.py
