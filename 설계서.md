...
![전체 구성도](<pictures/설계서/설계서 - 전체 구성도.png>)
...
- 학생 목록에서 학생을 클릭하면 해당 학생의 상세 정보(프로필·전투 정보·지형 적응도 등)가 표시된다.
- 스킬 버튼(EX·기본·강화·서브)을 클릭하면 해당 스킬의 이름과 설명이 팝업으로 표시된다.
...
![전체 화면 구조](<pictures/설계서/설계서 - 4.1 전체 화면 구조.png>)
...
![학생 도감 화면](<pictures/설계서/설계서 - 4.2 학생 도감 화면.png>)
...
![가챠 시뮬레이터 화면](<pictures/설계서/설계서 - 4.3 가챠 시뮬레이터.png>)
...
![육성 시뮬레이터 화면](<pictures/설계서/설계서 - 4.4 육성.png>)
...
- 비용 계산 버튼 클릭 → 필요 재화 테이블 갱신 + DB 저장

---

## 5. Conceptual Design

![ERD - Crow's Feet Diagram](<pictures/설계서/설계서 - erd.png>)
...
-- 가챠 배너의 정보 테이블 (3성 학생 1명당 1개, 앱 초기화 시 자동 생성)
...
-- 가챠 뽑기 기록 테이블 (banner ↔ student N:M 관계)
...
CREATE TABLE gacha_pull (
    ...
    pull_count  INTEGER NOT NULL    -- 뽑기 시점의 모집 누적 수
);

-- 현재 학생 캐릭터 육성 현황 테이블 (학생 획득 시 자동 생성)
...
CREATE TABLE cultivation (
    ...
    enhance_skill      INTEGER DEFAULT 1  CHECK (sub_skill BETWEEN 1 AND 10),
    sub_skill          INTEGER DEFAULT 1  CHECK (passive_skill BETWEEN 1 AND 10),
...
-- 학생 캐릭터 목표 육성 현황 테이블 (목표 설정 시에만 생성)
...
CREATE TABLE cultivation_goal (
    ...
    ex_skill           INTEGER DEFAULT 1  CHECK (ex_skill        BETWEEN 1 AND 5),
    normal_skill       INTEGER DEFAULT 1  CHECK (normal_skill    BETWEEN 1 AND 10),
    passive_skill      INTEGER DEFAULT 1  CHECK (passive_skill   BETWEEN 1 AND 10),
...
### 7.1 전체 학생 목록 조회 및 필터링 (Use Case 3.1)
...
### 7.2 학생 상세 정보 및 스킬 조회 (Use Case 3.2)
...
### 7.3 가챠 뽑기 실행 및 결과 저장 (Use Case 3.3)
...
### 7.4 보유 학생 현황 통합 조회 (Use Case 3.4)
...
### 7.5 육성 목표 설정 및 비용 계산 (Use Case 3.5)
...
- 실제 게임의 가챠 확률표(3성 3%, 픽업 0.7%, 200회 천장)를 분석하여 시뮬레이터에 반영할 예정이다.
...
- 학생 캐릭터 이미지는 SchaleDB의 이미지 URL(`https://schaledb.com/images/student/collection/{path_name}.webp`)을 DB의 `path_name` 컬럼에서 조합하여 동적으로 로드한다.
