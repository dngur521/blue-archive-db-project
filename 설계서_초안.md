...
> 참고: 사용자-Flet Page-Service-Repository-DuckDB 구조로 작성
...
sequenceDiagram
    ...
    GachaRepo->>DB: SELECT COUNT(*) FROM gacha_pull<br/>WHERE banner_id = ? AND reset_at 이후
    DB-->>GachaRepo: 현재 모집 카운트
    GachaRepo-->>Service: int

    loop 10회 반복 (10번째는 1성 제외 확률)
        Service->>Service: gacha_algorithm(pull_count, is_10th)
...
sequenceDiagram
    ...
    QueryRepo->>DB: SELECT s.full_name, s.star_grade,<br/>  c.star, c.eleph_count, c.level,<br/>  c.acquired_at,<br/>  cg.star, cg.level<br/>FROM cultivation c<br/>JOIN student s ON s.id = c.student_id<br/>LEFT JOIN cultivation_goal cg ON cg.student_id = c.student_id<br/>ORDER BY c.acquired_at DESC
    DB-->>QueryRepo: 결과 행 목록
    QueryRepo-->>Service: pd.DataFrame
...
sequenceDiagram
    ...
    DB-->>CultivRepo: 현재 육성 현황 (없으면 빈 행)
    CultivRepo-->>Service: CultivationDTO
    Service-->>View: CultivationDTO
    View-->>사용자: 현재 수치 필드 자동 입력

    사용자->>View: 목표 수치 입력 후 [비용 계산] 클릭
    View->>Service: calculate_and_save(student_id, targets)
    Service->>Service: compute_costs(current, targets, costs_table)
    Service->>CultivRepo: upsert(student_id, cur_*, tgt_*)
    CultivRepo->>DB: INSERT OR REPLACE INTO cultivation ...
    DB-->>CultivRepo: OK
